Mircoin Core version *0.15.1* is now available from:

  <https://download.mircoin.org/mircoin-0.15.1/>
...
A number of changes to the way Mircoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 mirs per kB (0.00001 MIR). This is to prepare for dropping the minimum transaction fee to 0.1 mirs per kB (0.0001 MIR) in the next release.
